import React from 'react'
import { motion, useInView, easeInOut } from "framer-motion";
import { useEffect, useState, useRef } from 'react';
import Button from './Button';

function SignUp() {
    const ref = useRef(null);
    const isInView = useInView(ref, { amount: 0.8 });
    const [showHeading, setShowHeading] = useState(false);

    useEffect(() => {
        if (isInView) {
            setShowHeading(true);
        }
    }, [isInView]);

    return (
        <div ref={ref} className='flex justify-end h-[80vh] md:h-[70vh] w-screen pt-20 px-8'>
            <div className='flex bg-signup bg-left md:bg-top bg-cover h-full rounded-t-3xl md:rounded-t-[70px] justify-center w-full items-center'>

                <motion.div className='flex flex-col justify-center items-center' initial={{ opacity: 0 }} animate={{ opacity: showHeading ? 1 : 0 }} transition={{ duration: 2, easeInOut }}>
                    <h2 className='flex animate-gradient mb-3'>Join us now.</h2>
                    <p className='flex text-center text-xs w-2/3 lg:w-[40%] mb-6'>This is your chance to be part of a
                        vibrant community that&apos;s shaping the future of AI.
                        Secure your spot today.</p>
                    <Button text={"Register Now"}/>
                </motion.div>

            </div>
        </div>
    )
}

export default SignUp