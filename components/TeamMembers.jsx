...
function TeamMembers({ exitFunction }) {
  ...
  const handleSubmit = async () => {
    ...
    for (const [index, member] of finalData.members.entries()) {
      console.log(
        `Member ${index + 1} degreeType:`,
        member.degreeType,
        "| All:",
        member,
      );
      const { uni, email, ntuEmail, matricNo, tele, school } = member;
      const participantType = member.participantType || (uni ? "uni" : "");
      ...
      const requiredFields = [
        { field: "name", message: `Name is required for member ${index + 1}.` },
        {
          ...
          message: `Participant type is required for member ${index + 1}.`,
        },
        {
          field: "email",
          message: `Email is required for member ${index + 1}.`,
        },
        {
          field: "gender",
          message: `Gender is required for member ${index + 1}.`,
        },
        {
          field: "tele",
          message: `Telegram handle is required for member ${index + 1}.`,
        },
        {
          field: "course",
          message: `Course / Stream / Track is required for member ${index + 1}.`,
        },
        {
          field: "nationality",
          message: `Nationality is required for member ${index + 1}.`,
        },
        {
          field: "diet",
          message: `Dietary preference is required for member ${index + 1}.`,
        },
        {
          field: "size",
          message: `T-shirt size is required for member ${index + 1}.`,
        },
      ];
      ...
      if (isUni) {
        if (!member.uni || member.uni.trim() === "") {
          setErrorText(`University is required for member ${index + 1}.`);
          cancelSubmission();
          return;
        }

        if (!member.degreeType || member.degreeType.trim() === "") {
          setErrorText(`Degree type is required for member ${index + 1}.`);
          cancelSubmission();
          return;
        }

        if (!member.year || member.year.trim() === "") {
          setErrorText(`Year is required for member ${index + 1}.`);
          cancelSubmission();
          return;
        }
      }
      ...
      if (isPreUni) {
        const preUniRequiredFields = [
          {
            ...
            message: `Institution name is required for member ${index + 1}.`,
          },
          {
            field: "preUniCategory",
            message: `Pre-uni category is required for member ${index + 1}.`,
          },
          {
            field: "expectedGradYear",
            message: `Expected graduation year is required for member ${index + 1}.`,
          },
          {
            field: "dateOfBirth",
            message: `Date of birth is required for member ${index + 1}.`,
          },
        ];
        ...
        if (!member.indemnityMsFormConfirmed) {
          setErrorText(
            `Please upload the signed indemnity form via the Microsoft Form and tick the confirmation box for member ${index + 1}.`,
          );
          cancelSubmission();
          return;
        }
      ...
      if (isUnder18) {
        const guardianRequiredFields = [
          {
            field: "guardianName",
            message: `Parent/Guardian name is required for member ${index + 1}.`,
          },
          {
            field: "guardianEmail",
            message: `Parent/Guardian email is required for member ${index + 1}.`,
          },
          {
            field: "guardianPhone",
            message: `Parent/Guardian phone is required for member ${index + 1}.`,
          },
        ];
        ...
        if (!member.guardianConsent) {
          setErrorText(
            `Parent/guardian consent is required for member ${index + 1}.`,
          );
          cancelSubmission();
          return;
        }
      ...
      if (isUni && uni === "Nanyang Technological University") {
        if (!matricNo || matricNo.trim() === "") {
          setErrorText(
            `Matriculation number is required for member ${index + 1}.`,
          );
          cancelSubmission();
          return;
        }
      ...
      if (member.guardianEmail && !emailRegex.test(member.guardianEmail)) {
        setErrorText(`Invalid parent/guardian email for member ${index + 1}.`);
        cancelSubmission();
        return;
      }
      ...
      if (tele && !teleRegex.test(tele)) {
        setErrorText(
          `Invalid Telegram handle for member ${index + 1}. It must be at least 5 characters long and can only contain letters, numbers, and underscores.`,
        );
        cancelSubmission();
        return;
      }
      ...
      if (member.guardianPhone && !phoneRegex.test(member.guardianPhone)) {
        setErrorText(
          `Invalid parent/guardian phone number for member ${index + 1}.`,
        );
        cancelSubmission();
        return;
      }
  ...
  return (
    <div className="relative flex flex-col gap-8 w-screen items-center px-8 md:px-12 overflow-hidden">
      ...
      <input
        ...
        name="teamName"
        value={teamName}
        className="outline-none w-full overflow-scroll placeholderIndicator bg-transparent text-[30px] md:text-[36px] lg:text-[64px] font-montserrat font-semibold caret-[#525562] tracking-tighter text-center text-slate-50"
      />
      <p className="flex text-center text-[9px] lg:text-xs px-14 tracking-wider lg:px-0 w-full md:w-1/2 lg:w-[30%] text-[#b9c2e2] mb-8">
        You can have up 2-5 members in your team. Add members using the button
        underneath this text and click the X icon to delete them.
        <br />
        <br /> The topmost profile card will automatically be designated as the
        team leader. Make sure to enter your team name above&#33;
      </p>
      {canAddMembers ? (
        <button
          onClick={addMember}
      ...
      >
        {members.map((member, index) => (
          <motion.div
            ...
            animate={{
              opacity: deletingId === member.id ? 0 : 1,
              x: deletingId === member.id ? 500 : 0,
            }}
